    ...
    return(
        <section className="w-full h-[28rem] group relative overflow-hidden !transition-all !duration-500 !ease-in-out p-4">
            ...
            <div className="absolute top-0 bottom-0 left-0 right-0 bg-white flex flex-col items-start justify-center gap-4 group-hover:bg-black group-hover:bg-opacity-60 !transition-all !duration-500 !ease-in-out p-4">
                <p className="text-2xl font-bold group-hover:text-gray-500 !transition-all !duration-500 !ease-in-out">{item.title}</p>
                <p className="text-lg group-hover:text-gray-500 !transition-all !duration-500 !ease-in-out">{item.description}</p>

                <h1 className="text-5xl font-bold group-hover:text-gray-500 group-hover:scale-150 !transition-all !duration-500 !ease-in-out">{item.heading}</h1>
            </div>
        </section>
    )
}